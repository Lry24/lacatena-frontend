...
export default function StockPage() {
  ...
  const movColumns: Column<Movement>[] = [
    ...
    { key: 'product_name', label: 'Produit', render: (r) => <span style={{ color: '#f0ead2' }}>{r.product_name ?? '—'}</span> },
    { key: 'variant_sku', label: 'SKU', render: (r) => <span style={{ fontSize: 11, fontFamily: 'monospace', color: 'rgba(240,234,210,0.5)' }}>{r.variant_sku ?? '—'}</span> },
    {
    ...
    { key: 'stock_before', label: 'Avant', render: (r) => <span style={{ color: 'rgba(240,234,210,0.5)' }}>{r.stock_before ?? '—'}</span> },
    { key: 'stock_after', label: 'Après', render: (r) => <span style={{ color: 'rgba(240,234,210,0.7)' }}>{r.stock_after ?? '—'}</span> },
    { key: 'reason', label: 'Raison', render: (r) => <span style={{ color: 'rgba(240,234,210,0.5)', fontSize: 12 }}>{r.reason || '—'}</span> },
    { key: 'created_at', label: 'Date', render: (r) => <span style={{ color: 'rgba(240,234,210,0.4)', fontSize: 12 }}>{fmtDate(r.created_at)}</span> },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <h1 style={{ fontSize: 20, fontWeight: 600, color: '#f0ead2', margin: 0 }}>Gestion du stock</h1>
                ...
                <tbody>
                  {alerts.map((a, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(201,99,74,0.06)' }}>
                      <td style={{ padding: '10px 12px', color: '#f0ead2', fontWeight: 500 }}>{a.product_name}</td>
                      <td style={{ padding: '10px 12px', fontFamily: 'monospace', fontSize: 11, color: 'rgba(240,234,210,0.5)' }}>{a.sku}</td>
                      ...
                      <td style={{ padding: '10px 12px', color: 'rgba(240,234,210,0.4)' }}>{a.stock_alert}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div style={{ background: '#1e2411', border: '1px solid rgba(240,234,210,0.08)', borderRadius: 10, padding: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <h2 style={{ fontSize: 12, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(240,234,210,0.4)', margin: 0 }}>Mouvements de stock</h2>
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}
            style={{ padding: '7px 12px', background: '#0f1209', border: '1px solid rgba(240,234,210,0.1)', borderRadius: 8, color: '#f0ead2', fontSize: 12, outline: 'none' }}>
      ...
      <Modal open={movModal} onClose={() => setMovModal(false)} title="Nouveau mouvement de stock" width={480}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, color: 'rgba(240,234,210,0.5)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 7 }}>UUID de la variante *</label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, color: 'rgba(240,234,210,0.5)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 7 }}>Type de mouvement *</label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, color: 'rgba(240,234,210,0.5)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 7 }}>Quantité *</label>
            <input type="number" min="1" value={movForm.quantity} onChange={(e) => setMovForm(p => ({ ...p, quantity: e.target.value }))} style={inputStyle} />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 11, color: 'rgba(240,234,210,0.5)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 7 }}>Raison</label>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          <button onClick={() => setMovModal(false)} style={{ flex: 1, padding: '10px', background: 'none', border: '1px solid rgba(240,234,210,0.1)', borderRadius: 8, color: 'rgba(240,234,210,0.6)', cursor: 'pointer', fontSize: 13 }}>Annuler</button>
          <button onClick={handleCreateMovement} disabled={saving} style={{ flex: 1, padding: '10px', background: '#E8B96A', border: 'none', borderRadius: 8, color: '#1a1f0e', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>
